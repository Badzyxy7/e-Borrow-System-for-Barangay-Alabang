...
// UPDATED QUERY: Added group_request_id and grouped items
...
// ORDER BY priority DESC ensures funeral requests (priority=1) appear first, then by creation date
$sql .= " ORDER BY br.priority DESC, br.created_at DESC";
$result = $conn->query($sql);

// Count prioritized requests for badge
$priority_count = $conn->query("SELECT COUNT(*) as c FROM borrow_requests WHERE status='pending' AND priority=1")->fetch_assoc()['c'];
...
// Merge grouped and individual requests for display
$display_requests = [];

// Add group requests first (they should maintain priority sorting)
...
usort($display_requests, function($a, $b) {
    ...
    return strtotime($bDate) - strtotime($aDate); // DESC
});
...
<div class="p-4">
    <!-- Compact Header with Priority Badge -->
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-bold text-gray-800 flex items-center gap-2">
            <i data-feather="clock" class="w-5 h-5 text-yellow-600"></i>
            Pending <span class="text-sm font-normal text-gray-500">(<?= $total_count ?>)</span>
            <?php if ($priority_count > 0): ?>
                <span class="ml-2 px-2 py-1 bg-purple-100 text-purple-700 text-xs font-semibold rounded-full flex items-center gap-1">
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-semibold text-gray-700">Requested</th>
                ...
                <?php foreach ($display_requests as $request): ?>
                    <?php if ($request['type'] === 'group'): ?>
                        ...
                        <tr class="hover:bg-gray-50 transition <?= $isPriority ? 'bg-purple-50' : '' ?> border-l-4 border-blue-500">
                            <!-- Equipment Column - Show "Group Request" -->
                            <td class="px-4 py-3">
                                ...
                                <div class="text-xs text-gray-700">
                                    <?php if ($isPriority): ?>
                                        <span class="font-semibold text-purple-700 flex items-center gap-1">
                                            ...
                                            <?= htmlspecialchars($mainData['purpose']) ?>
                                        </span>
                                    <?php else: ?>
                                        <?= htmlspecialchars($mainData['purpose'] ?? 'Not specified') ?>
                                    <?php endif; ?>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="text-xs text-gray-600">
                                    <?= date('M d, Y', strtotime($mainData['created_at'])) ?>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-2">
                                    <button onclick='openGroupDetailsModal(<?= json_encode($group) ?>, "pending")'
                                    ...
                                    <button onclick='openGroupApprovalModal(<?= json_encode($items) ?>, "<?= $request['group_id'] ?>")'
                                            class="px-3 py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition flex items-center gap-1 text-xs"
                                            title="Approve All">
                                        <i data-feather="check" class="w-3 h-3"></i>
                                    </button>
                                    <button onclick='openGroupRejectModal("<?= $request['group_id'] ?>", <?= json_encode(array_column($items, "id")) ?>)'
                                            class="px-3 py-1.5 bg-red-600 text-white rounded-lg hover:bg-red-700 transition flex items-center gap-1 text-xs"
                                            title="Reject All">
                        ...
                        <tr class="hover:bg-gray-50 transition <?= $row['priority'] == 1 ? 'bg-purple-50' : '' ?>">
                            ...
                            <td class="px-4 py-3">
                                <div class="text-xs text-gray-700">
                                    ...
                                    <?php else: ?>
                                        <?= htmlspecialchars($row['purpose'] ?? 'Not specified') ?>
                                    <?php endif; ?>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-2">
                                    <button onclick='openDetailsModal(<?= json_encode($row) ?>, "pending")'
                                            class="px-3 py-1.5 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition flex items-center gap-1 text-xs"
                                            title="View Details">
                                        <i data-feather="eye" class="w-3 h-3"></i>
                                    </button>

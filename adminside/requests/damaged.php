...
$sql = "SELECT br.*, e.name AS equipment_name, e.description AS equipment_desc, e.image AS equipment_photo,
        ...
        bl.actual_pickup_date, bl.actual_return_date, bl.return_photo, bl.payment_photo,
        bl.is_damaged, bl.damage_fee, bl.damage_notes,
...
$total_fees = $conn->query("SELECT SUM(damage_fee) as total FROM borrow_logs WHERE is_damaged=1")->fetch_assoc()['total'] ?? 0;
$total_count = $conn->query("SELECT COUNT(*) as count FROM borrow_logs WHERE is_damaged=1")->fetch_assoc()['count'] ?? 0;
?>
...
<div class="p-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h3 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                ...
                <?= $show_history ? 'Damaged Items - Full History' : 'Damaged Items - Recent' ?>
            </h3>
            <div class="flex flex-wrap items-center gap-3 mt-2 text-sm">
                ...
                <span class="text-gray-500">
                    Total Fees: <span class="font-bold text-red-600">₱<?= number_format($total_fees, 2) ?></span>
                </span>
                <?php if (!$show_history): ?>
                <span class="text-gray-300">•</span>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <form method="get" class="flex gap-2">
                ...
                <input type="text" name="search" placeholder="Search equipment or user..." 
                       ...
                       class="px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-500 outline-none w-64">
    ...
    <div class="bg-gray-50 rounded-t-xl border border-gray-200 border-b-0">
        <div class="grid grid-cols-12 gap-4 p-4 text-sm font-semibold text-gray-700">
            ...
            <div class="col-span-2">Damage Info</div>
    ...
    <div class="border border-gray-200 rounded-b-xl bg-white divide-y divide-gray-200">
        ...
        <div class="grid grid-cols-12 gap-4 p-4 items-center hover:bg-gray-50 transition">
          <!-- Equipment Column -->
            ...
            <div class="col-span-2 flex items-center gap-2">
                <div class="w-8 h-8 bg-gradient-to-br from-red-400 to-red-600 rounded-lg flex items-center justify-center text-white font-bold text-xs flex-shrink-0">
            ...
            <div class="col-span-2">
                <div class="bg-red-50 rounded-lg p-2 border border-red-200">
                    <div class="flex items-center gap-1 mb-1">
                        <i data-feather="alert-triangle" class="w-3 h-3 text-red-600"></i>
                        <span class="text-xs font-semibold text-red-700">Damage Fee</span>
                    </div>
                    <p class="text-lg font-bold text-red-600">₱<?= number_format($row['damage_fee'], 2) ?></p>
                    <p class="text-xs text-gray-600 mt-1 line-clamp-2"><?= htmlspecialchars($row['damage_notes']) ?></p>
                    <div class="text-xs text-gray-500 mt-1">
                        Returned: <?= date('M d, Y', strtotime($row['actual_return_date'])) ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-3">
                <div class="bg-indigo-50 rounded-lg p-3 border border-indigo-100 max-h-32 overflow-y-auto">
                    <div class="space-y-2 text-xs">
                        ...
                        <div class="flex items-start gap-1">
                            ...
                            <div class="flex-1">
                                <div class="font-medium text-gray-700">Approved:</div>
                        ...
                        <div class="flex items-start gap-1 pt-1 border-t border-indigo-200">
                            ...
                            <div class="flex-1">
                                <div class="font-medium text-gray-700">Delivered:</div>
                        ...
                        <div class="flex items-start gap-1 pt-1 border-t border-indigo-200">
                            ...
                            <div class="flex-1">
                                <div class="font-medium text-gray-700">Returned:</div>
                        ...
                        <?php if (!empty($row['damaged_by_name'])): ?>
                        <div class="flex items-start gap-1 pt-1 border-t border-red-200 bg-red-50/50 -mx-3 -mb-3 px-3 pb-3 mt-2">
                            <i data-feather="alert-triangle" class="w-3 h-3 text-red-600 mt-0.5 flex-shrink-0"></i>
                            <div class="flex-1">
                                <div class="font-medium text-red-700">Marked Damaged:</div>
                                <div class="text-red-600 font-semibold"><?= htmlspecialchars($row['damaged_by_name']) ?></div>
                                <?php if (!empty($row['marked_damaged_at'])): ?>
                                <div class="text-red-500"><?= date('M d, g:i A', strtotime($row['marked_damaged_at'])) ?></div>
                                <?php endif; ?>
                            </div>
                        </div>
                        <?php endif; ?>
            ...
            <div class="col-span-3 flex flex-col gap-1">
                ...
                <button onclick="viewPhoto('<?= htmlspecialchars($row['return_photo']) ?>', 'Return Proof (Damage)')"
                        class="w-full px-3 py-1.5 bg-red-50 text-red-700 rounded-lg hover:bg-red-100 transition text-xs flex items-center justify-center gap-1.5">
                    <i data-feather="alert-circle" class="w-3 h-3"></i>
                    <span>Return</span>
                </button>
                <?php endif; ?>
                <?php if (!empty($row['payment_photo'])): ?>
                <button onclick="viewPhoto('<?= htmlspecialchars($row['payment_photo']) ?>', 'Payment Proof')"
    ...
    <div class="mt-6 text-center">
        <a href="?tab=damaged&history=1" 
           class="inline-flex items-center gap-2 px-6 py-3 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition">
            ...
            <span>View All <?= $total_count ?> Damaged Records</span>
    ...
    <div class="text-center py-16 bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl border border-green-100">
        ...
        <p class="text-gray-700 text-lg font-semibold mb-2">
            <?= $search ? 'No results found' : 'No damaged items recorded' ?>
        </p>
        <p class="text-gray-500 text-sm">
            <?= $search ? 'Try adjusting your search terms' : 'All equipment has been returned in good condition' ?>
        </p>
    </div>
    <?php endif; ?>
</div>
...
/* Custom scrollbar for audit trail */

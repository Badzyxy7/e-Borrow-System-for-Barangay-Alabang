<?php
// Make sure we have access to $conn
if (!isset($conn)) {
  include "../db.php";
}
...
<div class="flex-1 px-8 py-6">
  ...
  <!-- Search + Filter -->
  <form method="get" action="admin_dashboard.php" class="flex flex-col md:flex-row gap-4 mb-6" id="inventoryFilterForm">
          ...
          <?php if($result->num_rows > 0): ?>
            ...
            <tr class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-6 py-4 flex gap-3">
                ...
                <button data-delete-id="<?php echo $row['id']; ?>" 
                       class="delete-equipment text-red-600 hover:text-red-800 transition" title="Delete">
            ...
            <div id="editModal<?php echo $row['id']; ?>" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
              <div class="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-md relative">
                ...
                <form method="post" action="load_content.php?tab=inventory" enctype="multipart/form-data" class="space-y-4 equipment-form">
...
<script>
    ...
    form.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const navLink = document.querySelector('[data-tab="inventory"]');
  ...
  document.querySelectorAll('.delete-equipment').forEach(btn => {
    btn.addEventListener('click', async function() {
      ...
      try {
        const response = await fetch(`load_content.php?tab=inventory&delete=${id}`);
    ...
    filterForm.addEventListener('submit', function(e) {
      ...
      fetch(`load_content.php?${params}`)
        ...
        .then(html => {
          ...
          history.pushState({tab: 'inventory'}, '', `?${params}`);
        });
    });
  }
</script>